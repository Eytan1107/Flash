...
class Flash : JavaPlugin() {
    override fun onEnable() {
        ...
        instance = this
        sql.prepareStatement("create table if not exists data(uuid varchar(48), kills int default(0), deaths int default(0), time int default(0), primary key(uuid));").executeUpdate()
        sql.autoCommit = true
        saveDefaultConfig()
        try {
