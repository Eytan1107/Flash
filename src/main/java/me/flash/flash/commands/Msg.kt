...
class Msg : CommandExecutor {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<String>): Boolean {
        ...
        if (!sender.hasPermission("flash.msg.nice")) sender.sendMessage("&a(To $prefix${player.name}&a) ".color() + args.toMutableList().apply { removeAt(0) }.joinToString (" ")) else sender.sendMessage("&bTo $prefix${player.name}&8:$suffix ".color() + args.toMutableList().apply { removeAt(0) }.joinToString (" "))
        if (!player.hasPermission("flash.msg.nice")) player.sendMessage("&a(From $senderprefix${sender.name}&a) ".color() + args.toMutableList().apply { removeAt(0) }.joinToString (" ")) else player.sendMessage("&bFrom $senderprefix${sender.name}&8:$sendersuffix ".color() + args.toMutableList().apply { removeAt(0) }.joinToString (" "))
