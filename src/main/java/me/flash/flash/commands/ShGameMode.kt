    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        ...
        if (args.isEmpty()) {
            if (sender !is Player) sender.sendMessage("&cUsage: /gamemode [gamemode] [player]".prefix()).run { return true }
                ...
                sender.sendMessage(("You set your gamemode to &l${gameMode.name.toLowerCase()}&6.").prefix())
            ...
            sender.sendMessage("You set &l${player.name}&6's gamemode to &l${gameMode.name.toLowerCase()}&6.".prefix())
            player.sendMessage("&l${sender.name} &6set your gamemode to &l${gameMode.name.toLowerCase()}&6.".prefix())
